...
if (!empty($user_email)) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['rating'], $_POST['cast_id'], $_POST['comment'])) {
        ...
        $check_sql = "SELECT * FROM castrating WHERE UserEmail='$user_email' AND CastID=$cast_id";
    ...
    <script>
     document.getElementById('customAlert').style.display = 'block';
            ...
            $sql = "INSERT INTO castrating (UserEmail, CastID, Rating, Comment) VALUES ('$user_email', $cast_id, $rating, '$comment')";
        ...
        <?php if ($result->num_rows > 0): ?>
            <?php while($row = $result->fetch_assoc()): ?>
                <div class="movie">
                    ...
                    $image_path = '../moviePersons/cast/images/' . $image_filename;
                    ?>
                    <img src="<?php echo $image_path; ?>" alt="<?php echo htmlspecialchars($row['Name']); ?>" width='200'>

                    <form class="rating-form" method="POST" action="">
                        <div class="stars">
                            <input type="radio" id="star5_<?php echo $row['CastID']; ?>" name="rating" value="5" required /><label for="star5_<?php echo $row['CastID']; ?>">&#9733;</label>
                            <input type="radio" id="star4_<?php echo $row['CastID']; ?>" name="rating" value="4" required /><label for="star4_<?php echo $row['CastID']; ?>">&#9733;</label>
                            <input type="radio" id="star3_<?php echo $row['CastID']; ?>" name="rating" value="3" required /><label for="star3_<?php echo $row['CastID']; ?>">&#9733;</label>
                            <input type="radio" id="star2_<?php echo $row['CastID']; ?>" name="rating" value="2" required /><label for="star2_<?php echo $row['CastID']; ?>">&#9733;</label>
                            <input type="radio" id="star1_<?php echo $row['CastID']; ?>" name="rating" value="1" required /><label for="star1_<?php echo $row['CastID']; ?>">&#9733;</label>
                        </div>
                        <label for="comment_<?php echo $row['CastID']; ?>">Comment:</label>
                        <textarea name="comment" id="comment_<?php echo $row['CastID']; ?>" rows="4" cols="50" required></textarea>
                        <input type="hidden" name="cast_id" value="<?php echo $row['CastID']; ?>">
