...
include 'adminedit.php'; // Include the userEdit.php file which contains HTML form
...
if (isset($_POST['submit'])) {
    ...
    $sql_select_image = "SELECT Image FROM adminsignup WHERE Email='$admin_email'";
    ...
    $sql_update_profile = "UPDATE adminsignup SET Fullname=?, Phone=?, DOB=?, Gender=?, Address=?, Image=? WHERE Email=?";
    $stmt_update_profile = $conn->prepare($sql_update_profile);
    $stmt_update_profile->bind_param("sssssss", $fullname, $phone, $dob, $gender, $address, $image, $admin_email);
